...
def generate_random_shop(shop_size, shop_type):
  ...
  common_items = [i for i in items if i.rarity == "C"]
  ...
  return list(chain(Item.objects.filter(category='ARM'), Item.objects.filter(category='WPN'), Item.objects.filter(category='SCR')))
